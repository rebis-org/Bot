...
import { entity, nonEmpty, record, roomId, userId } from './value.ts';
...
> = [
  v.check(
    ...
    'check-out time and duration must be either both set or both null'
  ),
  v.check(
...
const checkInFields = {
  ...
  userId: userId(),
  name: nonEmpty('name'),
  clockInAt: nonEmpty('check-in time')
} satisfies v.ObjectEntries;

const checkOutFields = {
  roomId: roomId(),
  userId: userId(),
  clockOutAt: nonEmpty('check-out time')
} satisfies v.ObjectEntries;
...
const workSchema = v.pipe(
  v.object({ id: v.number(), ...workFields }),
  ...sessionChecks
);
